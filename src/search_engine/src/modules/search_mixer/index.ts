...
export async function search_mixer({
	...
	const unknown_candidates_list = await unknown_candidates({user_id, search_term, related_candidates_list})
	const mixed_list = mix_lists(related_candidates_list, unknown_candidates_list, search_engine_config.MIX_COEFFICIENT)
    const candidates_with_security = security_filter({candidates: mixed_list})
