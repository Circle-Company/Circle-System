...
const generateHashKey = (fileName) => {
...
export async function upload_image_AWS_S3({imageBase64, bucketName, fileName}: UploadImageAWSS3Props) {
    try {
        ...
        .then(hashKey => { return `${hashKey}` })
        .catch((err) => { throw new ServiceError({message: err.message})})
