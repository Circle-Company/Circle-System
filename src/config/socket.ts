...
export function initializeSocket({
  ...
  io.on('connection', async (socket: any) => {
    ...
    socket.on('sign', async (props: any) => {
      const { user_id } = props
      // Associar o usuário ao socket
  
      // Aqui você pode realizar a autenticação usando o token, verificar se o usuário existe, etc.
      // Se a autenticação for bem-sucedida, associe o socket ao usuário e armazene no banco de dados.
      try {
