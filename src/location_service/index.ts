...
import { Coordinates } from "../models/user/coordinate-model.js"; // Modelo Sequelize para a tabela 'coordinates'
...
export async function LocationService({ user, radiusInKm }: LocationServiceProps): Promise<UserProps[]> {
    ...
    const lonDelta = radiusInKm / (RADIUS_OF_EARTH_KM * Math.cos((user.coordinates.latitude * Math.PI) / 180));
    ...
    const populatedUsers = nearbyUsers.map((user) => {
