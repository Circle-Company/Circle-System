...
export default async function interaction_tags_algorithm({
    ...
    if (tags_with_weights.length > 0) {
        ...
        const moments_with_tag = await MomentTag.findAll({
            where: {
                tag_id: maxWeightTag?.tag_id,
                moment_id: { [Op.notIn]: interacted_moments_list },
            },
        ...
        if (!secure_moments) return null
            ...
            const interaction_with_moments = await Promise.all(
                secure_moments.map(async (i) => {
            ...
            const moments_with_score = grouped_interactions.map((group) => {
                ...
                const momentScore =
                    interactions_scores.length > 0
                        ? interactions_scores.reduce((acc, curr) => acc + curr, 0) /
                          interactions_scores.length
                        : 0
            ...
            moments_with_score.forEach((moment) => {
                if (!highestScoreMoment || moment.score > highestScoreMoment.score)
                    highestScoreMoment = moment
            })
