...
export default async function negative_content_algorithm({
    ...
    if (a.length < 50) {
        const similarUsersInteractionsIdsList = similarUsersInteractions.map((i) => {
            return i.map((i: any) => i.id)
        })
        //@ts-ignore
        const unknownInteractions = await Interaction.findAll({
            where: {
                id: { [Op.notIn]: similarUsersInteractionsIdsList.concat(interacted_moments_list) },
            },
            attributes: ["negative_interaction_rate", "user_id", "moment_id", "id"],
    ...
    const averagedInteractions = Object.entries(interactionMap).map(([key, value]) => {
        ...
        if (!interacted_moments_list.includes(moment_id))
            return {
                id,
                user_id,
                moment_id,
    ...
    const groupedByMomentId: {
        [key: number]: {
            id: number
            user_id: number
            negative_interaction_rate_average: number
            similarity: number
        }[]
    } = {}
    averagedInteractions.forEach((interaction) => {
        if (interaction) {
            ...
            const user_similarity =
                similarUsers.find((item) => item.user_id === user_id)?.similarity || 0
            groupedByMomentId[moment_id].push({
                id,
                user_id,
                negative_interaction_rate_average,
    ...
    const momentsInteractions = Object.entries(groupedByMomentId).map(
    ...
    */
    const scoredMoments = momentsInteractions.map((moment) => {
        ...
        const ids = moment.interactions.map((interaction) => {
