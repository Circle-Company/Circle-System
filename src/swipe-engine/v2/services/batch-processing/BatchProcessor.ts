...
export class BatchProcessor {
    ...
    private initEmbeddingServices(): void {
        ...
        if (
            ...
            repositories.userEmbeddingRepository
        ) {
    ...
    private async recalculateClusters(): Promise<void> {
        try {
            ...
            // Executar recálculo de clusters de usuários
            if (this.config.repositories.userEmbeddingRepository) {
                const userClustersResult = await this.clusterRecalculator.recalculateUserClusters(
                    this.config.repositories.userEmbeddingRepository,
                    {
                        epsilon: 0.3,
                        minPoints: 5,
                ...
                this.logger.info(
                    `Recalculados ${userClustersResult.clusters.length} clusters de usuários ` +
                        `com ${userClustersResult.metadata?.totalItems || 0} usuários processados`
                )
            }
